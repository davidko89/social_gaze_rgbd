...
class RecorderWithCallback:
    ...
    def run(self):
        ...
        # cmd = f"ffmpeg -y -channel_layout stereo -ac 2 -i D:/data/raw_data/tmp_audio.wav -i D:/data/raw_data/tmp_video.avi -s 1280x720 -fpsmax 30 -pix_fmt yuv420p D:/data/raw_data/{self.file_path}_raw_video.avi"
        ...
        # pickle.dump(color_video, open(f'D:/data/proc_data/{self.file_path}/{self.file_path}_color.pickle', 'wb'))
        # pickle.dump(depth_video, open(f'D:/data/proc_data/{self.file_path}/{self.file_path}_depth.pickle', 'wb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device',
                        type=int,
                        default=0,
                        help='input kinect device participant')
    ...
    if args.participant is not None:
        ...
        #assert f'{args.participant}_color.npy' not in os.listdir('D:/data/proc_data/color'), 'participant participant is duplicated.'
    ...
    if device < 0 or device > 255:
        print('Unsupported device participant, fall back to 0')
        device = 0
